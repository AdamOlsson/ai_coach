...
def paf_to_pose_cpp_callback(tup):
    heatmap, paf, config = tup
    res = paf_to_pose_cpp(heatmap, paf, config)
    print(res)
    return res


def poseEstimationParallell(model, config, video, device, remove_bg_objects=True):
    ...
    data = zip(pafs, heatmaps, repeat(config))
    with Pool(1) as p:
        frames = p.map(paf_to_pose_cpp_callback, data)
